...
(defn setup-blockchain
  ...
  ([chain-url access-token issuer-alias asset-alias current-i-id current-i-xpub]
   (if (or (= "" current-i-id) (= "" current-i-xpub))
     (setup-blockchain chain-url access-token issuer-alias asset-alias)
     {:i-key current-i-xpub :i-alias issuer-alias :i-id current-i-id :i-asset asset-alias})))
